...
export default function App() {
  ...
  const currentCard = SAMPLE_ANKI_CARDS[currentCardIndex];
  ...
  const extractMedicalTerms = (text: string) => {
    ...
    Object.keys(MOCK_DRUGS).forEach(drugName => {
      if (lowerText.includes(drugName.toLowerCase())) {
        drug = MOCK_DRUGS[drugName];
      }
    });

    // Check for diseases
    let disease: DiseaseInfo | undefined;
    Object.keys(MOCK_DISEASES).forEach(diseaseName => {
      if (lowerText.includes(diseaseName.toLowerCase())) {
        disease = MOCK_DISEASES[diseaseName];
      }
    });
  ...
  const handleCardChange = (index: number) => {
    ...
    const card = SAMPLE_ANKI_CARDS[index];
  ...
  const handleAgain = () => {
    ...
    if (newCount >= 2) {
      const questions = generatePracticeQuestions(cardId, currentCard.front + " " + currentCard.back);
      setPracticeQuestions(questions);
    }

    // Move to next card
    const nextIndex = (currentCardIndex + 1) % SAMPLE_ANKI_CARDS.length;
  ...
  const handlePrevCard = () => {
    const prevIndex = currentCardIndex === 0 ? SAMPLE_ANKI_CARDS.length - 1 : currentCardIndex - 1;
  ...
  const handleGenerateQuestions = () => {
    ...
    SAMPLE_ANKI_CARDS.forEach((card) => {
      const questions = generatePracticeQuestions(card.id, card.front + " " + card.back);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <Tabs defaultValue="study" className="w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <Tabs defaultValue={enabledTabs.overview ? "overview" : enabledTabs.drugs ? "drugs" : enabledTabs.anatomy ? "anatomy" : "clinicals"} className="w-full">
                  <TabsList className={`grid w-full grid-cols-${Object.values(enabledTabs).filter(Boolean).length}`}>
                    ...
                    {enabledTabs.clinicals && (
                      <TabsTrigger value="clinicals" className="text-xs">
          ...
          <TabsContent value="schedule" className="space-y-6">
            <SchedulePlanner totalCards={SAMPLE_ANKI_CARDS.length} />
          </TabsContent>
        </Tabs>
      </main>
      ...
      {/* Footer */}
      <footer className="bg-white border-t mt-16 pb-20">
